...
Lessons distilled from previous trades:
  0. flag shorts based only on recent lower closes as serious issue when RSI has rebounded from oversold and MACD histogram is improving/positive.
...
Lessons distilled from previous trades:
  0. when the critic flags contradictions in a 0 call but price, RSI, and MACD are mixed, keep 0; with clear price/MACD momentum, take that direction.
...
class LSequential(torch.nn.Module):
